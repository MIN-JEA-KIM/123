...
def scrollLog(req):
    ...
    if req.method == 'POST':
        if 'scroll' in req.POST.keys():
            ...
            new_scroll_data = ScrollData(ipaddr=ip, acstime=datetime.now(), url=req.get_full_path(),
                                         staytime=req.POST['deltaTime'], scroll=req.POST['scroll'])
...
def news_post(req, n_id):
    ...
    if req.user == article.id:
        user = True
        print("User is logged in : o")
        print(f"Username --> {req.user.id}")
    else:
        user = False
        print("User is not logged in : x")
    data['user'] = user
    ...
    if req.COOKIES.get(cookie_name) is not None:
        cookies = req.COOKIES.get(cookie_name)
        cookies_list = cookies.split('|')
        if str(n_id) not in cookies_list:
            response.set_cookie(cookie_name, cookies + f'|{n_id}', max_age=max_age, httponly=True, samesite='Lax')
            return response
    else:
        response.set_cookie(cookie_name, n_id, max_age=max_age, httponly=True, samesite="Lax")
        article.hits += 1
        article.save()
        return response
    ...
    if req.method == 'POST':
        ...
        if req.POST['password'] == req.POST['password1']:
            ...
            try:
                ...
                # 객체를 새로 생성
                user = Memberinfo(id=req.POST['id'], password=req.POST['password'], name=req.POST['name'],
                                  birth=req.POST['birth'], sex=req.POST['sex'], email=req.POST['email'],
